          ...
          {stats.map((stat, i) => (
            <div key={stat.label} className="flex items-center">
              <div className="flex items-center gap-2.5 px-6 py-2">
                <div className="flex items-center justify-center w-7 h-7 rounded-lg bg-primary-50 text-primary-600 shrink-0">
                  {stat.icon}
                </div>
                <div>
                  <span className="text-base font-bold text-slate-900">{stat.value}</span>
                  <span className="ml-1.5 text-sm text-slate-400">{stat.label}</span>
                </div>
              </div>
              {i < stats.length - 1 && (
